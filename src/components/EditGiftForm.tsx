...
interface EditGiftFormProps {
  gift: Gift;
  onUpdateGift: (id: string, giftData: Partial<Omit<Gift, 'id' | 'created_at' | 'updated_at' | 'is_available'>>) => Promise<any>;
  onCancel: () => void;
  submitting?: boolean;
}

export const EditGiftForm = ({ gift, onUpdateGift, onCancel, submitting = false }: EditGiftFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onUpdateGift(gift.id, formData);
      onCancel();
    } catch (error) {
      ...
      <CardHeader className="bg-gradient-to-r from-sage-50 to-sage-100 rounded-t-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Pencil className="h-6 w-6 text-sage-800" />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3">
            <Button
              type="submit"
              disabled={submitting}
              className="flex-1 bg-sage-600 hover:bg-sage-700 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-200 disabled:opacity-50"
            >
              {submitting ? (
                <>
